...
const RoundTwoAdmin = () => {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:5000/api/roundtwo/${id}`, {
        question,
        options,
        correctAnswer,
      });
  ...
  return (
    <div className="p-5 min-h-screen bg-gradient-to-bl from-gray-900 via-black to-gray-800 flex flex-col items-center justify-center px-6 text-white">
      <motion.h1
        className="text-4xl md:text-5xl font-extrabold mb-8 text-center text-transparent bg-clip-text bg-gradient-to-r from-green-500 via-blue-500 to-teal-500"
      ...
      <div className="bg-gray-800 p-6 rounded-lg shadow-lg border border-gray-700 max-w-lg w-full">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="w-full p-3 bg-green-500 text-white font-semibold rounded hover:bg-green-600"
        >
          Update Question
        </motion.button>
      </div>
